...
#include "GpsDriver/GPSProvider.h"
class LinkManager;
...
public slots:
     ...
     **/
     void writeBytesSafe(const char *bytes, int length)
     {
        ...
        this->thread()->setPriority(Priority::HighestPriority);
        for (;;)
        {
            auto delta = GPSProvider::getCurrentTime() % (1000000 / 5);//5Hz
            if (delta < 10 * 1000)//10ms时间间隙
            {
                emit _invokeWriteBytes(QByteArray(bytes, length));
                break;
            }
